...
typedef struct elemento{
	int hora;
	int sucursal;
	int dia; /*Un numero del 1 al 7 que representa el dia de la semana*/
};
ElementoLista;
...
int main(){
	...
	construir_lista(&ptrLunes);
	construir_lista(&ptrMartes);
	construir_lista(&ptrMiercoles);
	construir_lista(&ptrJueves);
	construir_lista(&ptrViernes);
	
	destruir_lista(&ptrLunes);
	destruir_lista(&ptrMartes);
	destruir_lista(&ptrMiercoles);
	destruir_lista(&ptrJueves);
	destruir_lista(&ptrViernes);
	return 0;
}
