...
int buscarPalabra(char* galletas,char* palabra,int cantidad,int* cromosoma,int pos,int n){
    ...
    for(int i=pos+1;i<n;i++){
        ...
        if(letraPalabra==cantidad) return 1;
    }
    return 0;
}
int main(){
    ...
    for(int i=0; i<n; i++){
            int cromosoma[10] = {0};
            if(buscarPalabra(galletas,palabra,cantidad,cromosoma,i,n)){
/*                 for(int j=0;j<n;j++){
