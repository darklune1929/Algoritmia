...
*/
int is_valid(int solution[], TOperation * operations[]){
	int total_stocks = 0;
	for (int i = 0; i < N; ++i)
	{
		if (solution[i] == 1){
...
*/
void update_solution(int solution[], TOperation * operations[], int final_solution[], 
...
* total a comprar.
*/
void optimize_profitability_rec(int solution[], int step, TOperation * operations[], 
								int final_solution[], int *stocks_quantity, double *final_profit){
	int i=0;
	if (step > N - 1) return;
	do {
		solution[step] = i;
		if (is_valid(solution, operations)){
			if (step == N - 1)
				update_solution(solution, operations, 
					final_solution, stocks_quantity, final_profit);
			else
				optimize_profitability_rec(solution, step+1, 
...
void show_report(TOperation * operations[], int final_solution[], int stocks_quantity, double final_profit){
...
int main(){	
	...
	optimize_profitability_rec(solution, step, operations, final_solution,
				&stocks_quantity, &final_profit);
	show_report(operations, final_solution, stocks_quantity, final_profit);
