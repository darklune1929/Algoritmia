/*
 * Laboratorio 1 (2021-1), pregunta 1
 * Desarrollado por Johan Baldeón
 * Datos del archivo "2021-1_L1_P1_data.txt":
13
7000
13
1 2 30 10000 7
...
int read_data(int *M_ptr, int *D_ptr, int *P_ptr, TVehicleTypeData ** vehicles_type) {
	...
	FILE *file = fopen(FILE_NAME, "r");
  	...
  	if (!feof(file)) {
		...
		if (result != 1) {
			printf("No se puede leer el precio del combustible en soles.");
			return -1;
		}		
  	}  	
...
void calc_totals(TVehicleTypeData **vehicles_types, int n, int M, int D, int P, int chromosome[], 
	...
	int i,j, vehicles_type_num=0, accum_vehicles, type_index;
...
void show_selected_vehicle_types(TVehicleTypeData **vehicles_types, int n, int D, int P, int final_chromosome[]) {
	int i, j, prev_num=0, accum_vehicles;
	double maintenance_cost, fuel_cost, prop_dist = D / SERVICE_KMS;
	printf("Los tipos de vehículos seleccionados son:\n");
	for (i=0; i< VEHICLES_TYPE_NUM; i++){
		accum_vehicles=0;
		maintenance_cost = 0;
		fuel_cost = 0;
...
int main(){
	...
	printf("M = %d, D = %d, P = %d\n", M, D, P);	
  	for (i=0; i<VEHICLES_TYPE_NUM; i++){
	    printf("Tipo %d:, capacidad: %d, rendimiento: %d, costo del servicio: %d, cant. de vehículos: %d", 
	    	vehicles_types[i]->id, vehicles_types[i]->loading_capacity, vehicles_types[i]->performance_km_gal,
	    	vehicles_types[i]->maintenance_cost, vehicles_types[i]->vehicles_number);
		printf("\n");
	}			
