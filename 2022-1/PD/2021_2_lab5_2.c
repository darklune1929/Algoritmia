...
int main(){
    ...
    printf("La solución para el drone: \n");
    for(int i=0;i<droneY;i++){
        for(int j=0;j<droneX;j++){
            printf("%02d ",droneAnswer[i][j]);
        }
        printf("\n");
    }
    ...
    printf("La solución para el robot almacén: \n");
    for(int i=0;i<storageY;i++){
    ...
    printf("Ingresar la posición final de robot almacén: ");
...
void solveDrone(int droneX,int droneY,int** droneBoard,int** droneAnswer){
    ...
    for(int j=1;j<droneY;j++){
        droneAnswer[j][0] = droneAnswer[j-1][0] + abs(droneBoard[j][0]-droneBoard[j-1][0]);        
    }
    for(int i=1;i<droneX;i++){
        for(int j=1;j<droneY;j++){
