...
int detectarCamino(int** tablero,int n,int m,int k,int x,int y,int* x_move,int* y_move,int* counter){
    int next_x,next_y,bt_return;
    if(x==n-1 && y== m-1 && (*counter) < 20){
        (*counter)++;
        printf("\n");
        printf("Solucion #%d",(*counter));
        printf("\n");
		...
		for (int t=0; t<8; t++){
			...
			if (is_safe(tablero,n,m,next_x,next_y)){
				...
				bt_return = detectarCamino(tablero,n,m,k+1,next_x,next_y,x_move,y_move,counter);
				if (bt_return)
					return 1;
				else{
...
int main() {
    ...
    int x=0,y=0;
    tablero[0][0] = 2;
    if(detectarCamino(tablero,n,m,3,x,y,x_move,y_move,&counter)){
