...
int calcular_maximo(struct Canal *canal,int i,double suma,int p,int n){
	...
	if(calcular_maximo(canal,i+1,suma+canal[i].niv_rec,p-canal[i].inversion,n)){
...
int main(){
	...
	printf("El nivel maximo de recordacion de la marca es de %d ",max);
	int tot = 0;
	double ing = 0;
	for(i=0;i<num;i++){
		tot += canal[sol[i]-1].inversion;
		ing += canal[sol[i]-1].inversion*canal[sol[i]-1].ROAS;
	}
	printf("con una inversion total en publicidad de %d.00 dolares ",tot);
	printf("y un ingreso de %.2lf dolares \n\n",ing);
	...
	for(i=0;i<num;i++){
		...
		printf("recordacion de marca = %d, ",canal[sol[i]-1].niv_rec);
