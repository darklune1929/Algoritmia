...
void calc_costs(TPhaseData **phases, int phase_number, int NF, int N, int ini_budget, int savings) {
	...
	if (budget <= 0 || (budget > 0 && total_cost == 0)){
		...
		calc_costs(phases, phase_number+1, NF, N, ini_budget, savings);		
	}
}

int main(){
	int budget, NF, N, ini_budget, phase_number=1, i,j, savings=0;
	TPhaseData ** phases;
	...
	*/
	...
	ini_budget = budget;
	calc_costs(phases, phase_number, NF, N, ini_budget, savings);	
	return 0;
}